...
testGetOperation = do
  state <- newIORef 123
  value <- runAction $ showNumber state
  if value == 123
...
testPutOperation = do
  state <- newIORef 0
  result1 <- runAction $ setNumber state 99
  result2 <- runAction $ setNumber state 99
...
testPostAddOperation = do
  state <- newIORef 10
  result1 <- runAction $ addNumber state 5
  result2 <- runAction $ addNumber state 5
...
testPostRandomiseOperation = do
  state <- newIORef 0
  result <- runAction $ randomiseNumber state
...
testDeleteOperation = do
  state <- newIORef 456
  result1 <- runAction $ resetNumber state
  result2 <- runAction $ resetNumber state
...
testNaturalConstraints = do
  state <- newIORef 5
  result <- runAction $ addNumber state 10
...
||| Test that HTTP operations have correct grades
testHandlerGrades : IO TestResult
testHandlerGrades = pure $
  let getGrade = httpGrade GET
      putGrade = httpGrade PUT
      postGrade = httpGrade POST
      deleteGrade = httpGrade DELETE
      correctGrades = getGrade == Safe && 
                      putGrade == Idempotent &&
                      postGrade == Unsafe &&
                      deleteGrade == Idempotent
  in if correctGrades
       then Pass "HTTP operations have correct grades"
...
main = do
  ...
  runTest "HTTP operation grade correctness" testHandlerGrades
